...
def extract_frame(vid_item, fps=1):
    ...
    if task == 'rgb':
        if args.use_opencv:
            ...
            try:
                ...
                frame_index = 1
                while len(vr) > 0 and vr[0] is None:
                    pass
                for i, vr_frame in enumerate(vr):
                    frame_offset = vr.fps // fps
                    if i % frame_offset != 0:
                        continue
                    if vr_frame is not None:
                        ...
                        mmcv.imwrite(out_img,
                                     f'{out_full_path}/img_{frame_index:05d}.jpg')
                        frame_index += 1
                    else:
                        warnings.warn(
                            'Length inconsistent!'
...
if __name__ == '__main__':
    ...
    if args.resume:
        import os
        done_vids = os.listdir(args.out_dir)
        done_fullpath_list = [args.src_dir + vid + '.mp4' for vid in done_vids]
        done_fullpath_list += [args.src_dir + vid + '.mkv' for vid in done_vids]

        done_fullpath_list = set(done_fullpath_list)

        fullpath_list = list(set(fullpath_list).difference(done_fullpath_list))
        print('Total number of videos to extract: ', len(fullpath_list))
