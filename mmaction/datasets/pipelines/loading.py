...
@PIPELINES.register_module()
class SelectorSampleFrames:
    def __init__(self, selector_model, feature_path, temperature, topk):
    ...
    def __call__(self, results):
        ...
        video_id = results['frame_dir'].split('/')[-1]
        feature_file = open(osp.join(self.feature_path, video_id + '.pkl'), 'rb')
        frame_features = pickle.load(feature_file)
        logits = self.model(torch.FloatTensor(frame_features).cuda())
        ...
        frame_indices = torch.topk(frame_weights.squeeze(), self.topk)[1].cpu().numpy() + results['start_index']
        results['frame_inds'] = frame_indices.astype(np.int)
...
@PIPELINES.register_module()
class IndexSampler:
    def __init__(self, frame_index_dict):
        self.frame_index_dict = frame_index_dict

    def __call__(self, results):
        total_frames = results['total_frames']

        video_id = results['frame_dir'].split('/')[-1]
        results['frame_inds'] = np.array(self.frame_index_dict[video_id])
        # results['frame_inds'] = frame_index_dict
        results['offset'] = 1
...
@PIPELINES.register_module()
class IndexSampleFrames:
    def __init__(self, frame_file):
        self.frame_dict = pickle.load(open(frame_file, 'rb'))

    def __call__(self, results):
        total_frames = results['total_frames']
        
        video_id = results['frame_dir'].split('/')[-1]
        results['frame_inds'] = self.frame_dict[video_id]
        results['num_clips'] = 1
        results['clip_len'] = len(self.frame_dict[video_id])
        # results['frame_inds'] = np.array(self.frame_index_dict[video_id])
        results['offset'] = 1
