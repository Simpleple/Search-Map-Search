...
class Decoder(nn.Module):
    ...
    def forward(self, embedded_inputs, seq_len,
                decoder_input,
                hidden,
                context, target=None):
        """
        ...
        mask = torch.arange(max_len).repeat(batch_size, 1).cuda()
        mask = mask < seq_len.view(-1, 1)
        ...
        for i in range(self.decode_len):
            ...
            if target is None:
                ...
                one_hot_pointers = (runner == target[:, i].unsqueeze(1).expand(-1, outs.size()[1])).float()
...
class PointerNet(nn.Module):
    ...
    def __init__(self, embedding_dim,
        ...
        self.encoder = Encoder(embedding_dim,
                               ...
                               dropout,
                               bidir)
        self.decoder = Decoder(embedding_dim, hidden_dim, decode_len)
        self.decoder_input0 = Parameter(torch.FloatTensor(embedding_dim), requires_grad=False)

        # Initialize decoder_input0
        nn.init.uniform(self.decoder_input0, -1, 1)

    def forward(self, inputs, seq_len, target=None):
        """
        ...
        embedded_inputs = inputs #self.embedding(inputs).view(batch_size, input_length, -1)
        ...
        if self.bidir:
            ...
            decoder_hidden0 = (encoder_hidden[0][-1],
                               encoder_hidden[1][-1])
        (outputs, pointers), decoder_hidden = self.decoder(embedded_inputs, seq_len,
                                                           decoder_input0,
                                                           decoder_hidden0,
                                                           encoder_outputs, target)
