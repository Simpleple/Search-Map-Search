...
class Recognizer2D(BaseRecognizer):
    ...
    def forward_train(self, imgs, labels, **kwargs):
        ...
        loss_cls = self.cls_head.loss(cls_score, gt_labels, reduction="none", **kwargs)
        losses.update(loss_cls)

        return losses#, feat

    def forward_train_features(self, imgs, labels, **kwargs):
        """Defines the computation performed at every call when training."""
        ...
        # loss_cls = self.cls_head.loss(cls_score, gt_labels, reduction="none", **kwargs)
        # losses.update(loss_cls)
        gt_prob = cls_score.softmax(dim=-1)[:, labels]
